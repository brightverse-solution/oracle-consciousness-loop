...
export async function propose(
  ...
  // Use --body-file to avoid shell escaping issues with large markdown
  ...
  try {
    ...
    const bodyFile = `${outbox_path}.gh-body.tmp`;
    await writeFile(bodyFile, body, "utf-8");

    const assignees = input.github_assignees?.join(",") ?? "";
    let cmd;
    if (assignees) {
      cmd = $`gh issue create --repo ${input.github_repo} --title ${title} --body-file ${bodyFile} --assignee ${assignees}`;
    } else {
      cmd = $`gh issue create --repo ${input.github_repo} --title ${title} --body-file ${bodyFile}`;
    }
    const url = await cmd.text();
    issue_url = url.trim();
    console.log(`[propose] GitHub issue: ${issue_url}`);

    // Clean up tmp body file
    await $`rm -f ${bodyFile}`.quiet();
  } catch (err) {
